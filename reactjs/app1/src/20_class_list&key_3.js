...
class Company extends React.Component {
    ...
    deleteCompany = () => {
        //alert(this.name + "will be deleted");
        this.props.deleteCompany(this.name);
    }
    render() {
        return (<tr>
            <td>{this.srno}</td>
                            ...
                            <table className="table table-bordered table-striped">
                                ...
                                <tbody>
                                    {this.state.sensexCompanies.map((item, index) => {
                                        return <Company no={index + 1} name={item.company} sector={item.sector} year={item.established} marketCap={item.marketCapCrore} key={item.company}
